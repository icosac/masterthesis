...
definition of \acrs{mapf}, the edge conflict should never happen since the 
...
\begin{figure}[t]
  ...
  \caption{On the left a \acrs{MAPF} problem with thee agents: the first agetn
...
\acrs{mapf} problem in $O(\abs{V}^3)$, but the solution it returns it may not
be the optimal one. \newline
...
Basically, the \acrs{MDD} has a single source node which corresponds with the
starting node of the agent. Then it keeps track of all the neighbors of the
source node adding them if the path going through them can lead to the final
node with cost $C_i$. This implies also that the \acrs{MDD} has a single sink
...
%
\subsection{\acrf{CBS}}
Similarly to \acrs{ICTS}, it uses two distinct search, a high-level and a
low-level one, and uses a tree to solve the \acrs{MAPF} problem. \newline
...
$t$ on the node $n$. Then the high-level search creates two new \acrs{CT} nodes
from the parent, one in which agent $a_i$ \textit{cannot} be on node $n$ at
time $t$, and the other \acrs{CT} node in which agent $a_j$ cannot be on node
$n$ at time $t$. \newline
...
plan. \newline
Finally, as it regards the low-level search, it can actually be any \acrs{SAPF}
...
that is mainly used when coding~\cite{CP}. \acrs{MILP} is a mathematical
...
one could use a \acrs{CP} solver or model the problem so that it can be solved
with \acrf{SAT} or \acrf{MIP} solvers. \newline
If the constraint are well-formed, i.e., they correctly cover the variables and
their domains, than \acrl{cp} is both optimal and correct. A brief description
of the constraint used in~\cite{picat} to solve the \acrs{MAPF} problem is now
...
results when applied to the \acrs{ICTS} algorithm proposing two new variations
called $\varepsilon-$\acrs{ICTS} and $\omega-$\acrs{ICTS}. \newline
...
\begin{figure}[htpb]
  ...
  \end{subfigure}
  \caption{A confront between \acrs{ICTS} for the classical \acrs{MAPF} and how
  the \acrs{MDD} is built for non unitary edges.}
  \label{fig:eICTS}
\end{figure}
