...
\begin{figure}[t]
  ...
  \caption{On the left a \acrs{MAPF} problem with thee agents: the first agent
...
example, one could use a \acrs{CP} solver or model the problem so that it can
be solved with \acrf{SAT} or \acrf{MIP} solvers. \newline
...
description of the constraint used in~\cite{picat} to solve the \acrs{MAPF}
